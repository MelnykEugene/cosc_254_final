...
class HashTree:
        ...
        if isinstance(current_node,LeafNode):
            ...
            if next_node is None:
                if k == (len(itemset.itemset) - 2):  # if this is true then the next node must be a leaf
                    ...
                    # set the new node to the head of linked and point to the previous last added node
            ...
            # if the next node does exist, simply advance the recursion
            self.insert(current_node.childs[direction], itemset, k + 1,verbose)

    # itemset here is ONLY an item array
    # no need for recusion, the hashtree can be be traversed iteratively
    def lookup(self, itemset):
        ...
        while k < len(itemset)-1:
            direction = itemset[k] % self.h
            next_node = current_node.childs[direction]
            if next_node is None:
                return False
        ...
        return itemset[-1] in leaf_itemsets_last_items
    ...
    def update(self, node, transaction, first_position, fixed):
        ...
        for i in range (first_position,last_possible_path+1):
            ...
            if isinstance(next_node,InteriorNode):
                ...
                for j in range(i + 1, last_possible_path + 1):
